...
export default function InvestigationMissionActions({ investigationId }: { investigationId: string }) {
  ...
  const selectedEvidenceIds=useMemo(()=>[...new Set(selectedFindings.flatMap(f=>f.evidence_ids??[]))].sort(),[selectedFindings]);
  ...
  },[investigationId,graph,selectedStepId,evidenceKey]);
  ...
  return <div className="missionGraphWorkspace">
    ...
    {focusedGraph&&selectedStep&&<section className="missionGraphFocus"><div className="missionGraphFocusHeader"><div><span className="labLabel">Mission → canonical graph</span><h3>{humanize(selectedStep.agent_id)} evidence footprint</h3><p>{projectionLoading?"Recomputing graph analytics for this mission evidence scope…":projectionError?`Projection unavailable; showing canonical graph fallback. ${projectionError}`:selectedEvidenceIds.length?`Showing a backend-recomputed graph projection backed by ${selectedEvidenceIds.length} evidence link(s) referenced by this step's ${selectedFindings.length} finding(s).`:"This step has no explicit evidence-linked findings yet, so the full canonical investigation graph remains visible."}</p></div><div className="missionGraphFocusStats"><span><b>{selectedFindings.length}</b> findings</span><span><b>{selectedEvidenceIds.length}</b> evidence</span><span><b>{focusedGraph.nodes.length}</b> nodes</span><span><b>{focusedGraph.edges.length}</b> edges</span></div></div><GalileoGraph graph={focusedGraph as any}/></section>}
  </div>;
}
