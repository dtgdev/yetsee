...
type EvidenceAccounting = {
  ...
  observation_evidence_count:number;
  literature_evidence_count:number;
  independent_source_count:number;
  independent_publication_count:number;
  supporting_count:number;
  contradicting_count:number;
  contextual_count:number;
  literature_supporting_count:number;
  literature_contradicting_count:number;
  literature_contextual_count:number;
  literature_items:{
    evidence_link_id:string;
    stance:string;
    weight:number;
    passage:{id:string;section:string;locator:string;text:string;content_hash:string;provenance:Record<string,unknown>};
    publication:{id:string;pmid:string|null;doi:string|null;title:string;journal:string|null;publication_date:string|null;source_url:string|null;content_hash:string}|null;
  }[];
  policy:{canonical_sources:string[];derived_claims_are_evidence:boolean;memory_is_evidence:boolean};
};
...
export default async function Page({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{lens?:string}>}){
  ...
  const observationSources=[...new Set(w.observations.map(o=>o.source))];
  const literatureSourceLabels=[...new Set(evidenceAccounting.literature_items.map(item=>item.publication?.pmid?`PubMed ${item.publication.pmid}`:item.publication?.title).filter((x):x is string=>Boolean(x)))];
  const sourceLabels=[...observationSources,...literatureSourceLabels];
  ...
  const canonicalEvidenceCount=evidenceAccounting.canonical_evidence_count;
  ...
  <GalileoGraph graph={investigationGraph}/>
    ...
    body=<div className="lensWorkspace agenticWorkspace">
      ...
      <ResearchMetrics>
        ...
        <Metric label="Canonical evidence" value={canonicalEvidenceCount} note={`${canonicalSourceCount} independent sources`} tone="violet"/>
      </ResearchMetrics>
      <div className="agenticAssessmentGrid">
        <ResearchPanel title="Current scientific assessment" subtitle="A deterministic summary of evidence, structure and reasoning state.">
          <div className="assessmentRows">
            ...
            <div><span>Independent sources</span><strong>{canonicalSourceCount} · {canonicalSourceCount>=3?"stronger coverage":"needs diversity"}</strong></div>
            ...
            <div><span>Counter-evidence</span><strong>{canonicalContradicting?`${canonicalContradicting} linked`:`None linked`}</strong></div>
          </div>
        </ResearchPanel>
